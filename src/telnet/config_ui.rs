    ...
    pub(in crate::telnet) fn security_help_lines(petscii: bool) -> &'static [&'static str] {
        if petscii {
            &[
                ...
                "  The password is stored as a",
                "  PBKDF2 hash - it cannot be read",
                "  back out of egateway.conf.",
                "",
            ...
            &[
                ...
                "  SSH, and the web configuration UI.  The",
                "  password is stored as a PBKDF2 hash, so it",
                "  cannot be read back out of egateway.conf.",
                "",
    ...
    pub(in crate::telnet) fn config_help_lines(petscii: bool) -> &'static [&'static str] {
        if petscii {
            &[
                ...
                "     gateway over SSH)",
                "  F  Test the ports: connect to",
                "     each one at this machine's",
                "     own address. A port that",
                "     does not answer is marked *",
                "     -- something local blocks it.",
                "     Answering proves less: on",
                "     Windows and macOS a self-",
                "     connection skips the",
                "     firewall, and nothing here",
                "     sees a router that is not",
                "     forwarding. Open the ports",
                "     on your firewall either way.",
                "",
            ...
            &[
                ...
                "  F  Test the ports.  Connects to each bound",
                "     listener at this machine's own network",
                "     address; one that does not answer is marked",
                "     with a * and is being blocked by something",
                "     on this machine.  Answering proves less than",
                "     it looks: on Windows and macOS a connection",
                "     to your own address skips the firewall",
                "     entirely, and nothing running here can see a",
                "     router that is not forwarding a port.  Open",
                "     the ports on your firewall either way.  Runs",
                "     once at start-up as well.",
                "",
