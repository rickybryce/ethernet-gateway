...
"""One automated transfer against NovaTerm, in either direction.
    ...
    run-transfer.py <protocol> <download|upload> [dial] [tg=<host>:<port>]

With `tg=` the run does not use the gateway it dialled: it takes that
gateway's **Telnet Gateway** out to <host>:<port> and runs the transfer on the
far one.  That is the master/slave telnet leg -- dial the slave, telnet to the
master -- and it means two terminal detections on the way in.
...
def main():
    ...
    nt.type('n', 3.0)
    if tg:
        # Hop to the far gateway through this one's Telnet Gateway.  The far
        # end then runs its OWN terminal detection and colour prompt, so both
        # are answered again -- and note what the far end will decide: a
        # translating gateway folds the C64's 0x14 erase byte to 0x7F on the
        # way out, so the master detects ANSI and sends 80-column menus to a
        # 40-column machine.  That is a property of the path, not a fault
        # here, and it is why the raw (`gateway_petscii_translate = false`)
        # configuration exists.
        host, _, port = tg.partition(':')
        nt.type('t', 3.0)
        nt.type(host + '\n', 3.0)
        nt.type((port or '23') + '\n', 8.0)
        nt.inst_del(3.5)
    ...
    # The gateway's negotiation window is 45 s and a waiting receiver's screen
    # is static, so a "nothing changed" verdict before then is a statement
    # about the harness, not about the transfer.  Let the product reach its own
    # outcome -- a transfer or a timeout it logs -- before believing silence.
    SETTLE_FLOOR = 70.0
    began = time.time()
    last, still = None, 0
    for _ in range(18):
        ...
        if body == last:
            still += 1
            if still >= 3 and time.time() - began >= SETTLE_FLOOR:
                break
        else:
            last, still = body, 0
    for l in nt.text():
