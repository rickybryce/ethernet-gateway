#!/usr/bin/env bash
# Bring up the TELNET rig for a HAND-DRIVEN transfer matrix.
#
#   VICE x64sc (NovaTerm, SwiftLink ACIA $DE00)
#        |  ip232 on TCP 25232
#      tcpser  (a virtual Hayes modem)
#        |  telnet TCP 2323
#   ethernetgateway
#
# The serial rig's socat PTY pair is not used here, and the gateway's serial
# port A is turned OFF -- on this link the C64 dials *tcpser*, not our own
# modem emulator.  tcpser auto-negotiates telnet + RFC 856 binary and the
# gateway turns IAC escaping on once telnet is negotiated
# (`xmodem_iac = telnet_negotiated`), so 8-bit blocks full of 0xFF and 0x0D
# survive.
#
# **Order matters and cannot be repaired later.**  tcpser must be up BEFORE
# VICE: VICE's ip232 side is a client that connects once and never retries, so
# restarting tcpser under a live emulator strands it -- which presents as a
# second, worse hang than the one being fixed.
#
...
# Serial port A off: this link does not use it, and leaving it on would have
# the gateway holding a PTY that no longer exists.
...
grep -q "Telnet server listening" gateway.log 2>/dev/null \
    && grep "Telnet server listening" gateway.log | tail -1

# tcpser BEFORE VICE -- see the note at the top.
TCPSER="${TCPSER:-$HOME/claude/punter-vice/tcpser/tcpser}" nohup ./start-tcpser.sh > tcpser.log 2>&1 &
for _ in $(seq 1 50); do
    ss -ltn 2>/dev/null | grep -q 25232 && break
    sleep 0.2
done
ss -ltn 2>/dev/null | grep -q 25232 || { echo "FATAL: tcpser is not listening on 25232; see tcpser.log" >&2; exit 1; }
